...
public class LocalSettingsMenu extends AbstractGUIMenu {
    ...
    public void setItems() {
        this.inv.setItem(11, makeItem(Material.REDSTONE_BLOCK, "&cDebug Logging", "", "&aDetermine whether ZetaCore should add in debug logs to console.", "&aThis can help when trying to find problems or reporting bugs", "&6" + ZetaCore.getBooleanColor(ZetaCore.INSTANCE.localSettings.shouldDebug)));
    ...
    public void handleClick(InventoryClickEvent e) {
        switch (e.getSlot()) {
            case 11:
                ZetaCore.INSTANCE.localSettings.shouldDebug = !ZetaCore.INSTANCE.localSettings.shouldDebug;
                this.refresh();
                break;
    ...
    public static class SQLConfigurationMenu extends AbstractGUIMenu implements IChatInputable {
        ...
        public void setItems() {
            ...
            this.inv.setItem(19, this.makeSkullWithCustomTexture("&aSet hostname", new String[]{"", "&6Set the IP address of the MySQL database", "&aDefault: localhost", "&aCurrent: " + ZetaCore.INSTANCE.localSettings.ipAddress}, "eyJ0ZXh0dXJlcyIgOiB7IlNLSU4iIDogeyJ1cmwiIDogImh0dHA6Ly90ZXh0dXJlcy5taW5lY3JhZnQubmV0L3RleHR1cmUvODEzYzlkOWJlNWRhNjFmMDJjYzBiZjI3MTcwNWU4NTZlYmUwMjc5Mjc0MGZlZmI0MTE5OWFmYmQyNzExYTQ5YSJ9fX0="));
            this.inv.setItem(21, this.makeItem(VersionMaterial.OAK_SIGN.getMaterial(), "&aSet username", "", "&6Set the username to login to the database", "&aDefault: root", "&aCurrent: " + ZetaCore.INSTANCE.localSettings.username));
            this.inv.setItem(23, this.makeItem(Material.OBSIDIAN, "&aSet login password", "", "&cType in NO to set the password to empty"));
            this.inv.setItem(25, this.makeItem(Material.CHEST, "&aSet database name", "", "&aCurrent: " + ZetaCore.INSTANCE.localSettings.databaseName));

            this.inv.setItem(31, this.makeItem(VersionMaterial.COMMAND_BLOCK.getMaterial(), "", "&aSet port", "&6Set the database's connection port", "&aDefault: 3306", "&aCurrrent: " + ZetaCore.INSTANCE.localSettings.port));

            if (ZetaCore.INSTANCE.localSettings.initialized) {
            ...
            switch (mat) {
                ...
                default:
                    if (mat.name().contains("WOOL") && e.getSlot() != 51) {
                        this.close();
                        if (ZetaCore.INSTANCE.localSettings.initialized) {
                            ZetaCore.INSTANCE.localSettings.initialized = false;
                            Bukkit.getScheduler().runTaskAsynchronously(ZetaCore.INSTANCE, () -> {
                                ZetaCore.INSTANCE.localSettings.client.disconnect();

                                if (!ZetaCore.INSTANCE.localSettings.client.isConnected()) {
                                ...
                                ZetaCore.INSTANCE.localSettings.client = null;
                            });
                        } else {
                            ZetaCore.INSTANCE.localSettings.initialized = true;

                            Bukkit.getScheduler().runTaskAsynchronously(ZetaCore.INSTANCE, () -> {
                                ZetaCore.INSTANCE.localSettings.client = new SQLClient(ZetaCore.INSTANCE.localSettings.ipAddress, ZetaCore.INSTANCE.localSettings.username, ZetaCore.INSTANCE.localSettings.password, ZetaCore.INSTANCE.localSettings.port, ZetaCore.INSTANCE.localSettings.databaseName);

                                LocalDateTime timeout = LocalDateTime.now().plus(3, ChronoUnit.SECONDS);
                                while (!ZetaCore.INSTANCE.localSettings.client.isConnected()) {
                        ...
                        Bukkit.getScheduler().runTaskAsynchronously(ZetaCore.INSTANCE, () -> {
                            ...
                            if (ZetaCore.INSTANCE.localSettings.client.isConnected()) {
                                ...
                                for (ZetaPlugin plugin : ZetaCore.getDataHandlers().keySet()) {
                                    List<DataStorer> dataStorerList = ZetaCore.getDataHandlers(plugin);
                                    if (plugin.getSettings().isSyncDatabase()) {
                                        for (DataStorer storer : dataStorerList) {
                                            if (storer instanceof ISQLTableHandler && !(storer instanceof ISQLTableHolder)) {
                                                ...
                                                tableHandler.setTable(new SQLTable(tableHandler.getTableName(), ZetaCore.INSTANCE.localSettings.client.handler));
                                                tableHandler.getTable().setPrimaryKey(tableHandler.getPrimaryKey());
                                                tableHandler.getTable().setColumns(tableHandler.getColumns(ZetaCore.INSTANCE.localSettings.client.handler));
                                                ((ISQLTableHandler<?>) storer).readDB();
                                            } else if (storer instanceof ISQLTableHolder) {
                                                ((ISQLTableHolder) storer).syncDB(ZetaCore.INSTANCE.localSettings.client.handler);
                                            }
                                        }
                                    }
                                }
        ...
        public void processChatInput(InputType type, AsyncPlayerChatEvent event) {
            ...
            switch (type) {
                ...
                case NUMBER:
                    try {
                        ...
                        if (port > 0 && port < 65536) {
                            ZetaCore.INSTANCE.localSettings.port = port;
                        } else {
                ...
                case STRING:
                    ZetaCore.INSTANCE.localSettings.ipAddress = event.getMessage();
                    owner.sendMessage(ChatColor.GREEN + "Setting hostname to " + event.getMessage());
                    break;

                case STRING1:
                    ZetaCore.INSTANCE.localSettings.username = event.getMessage();
                    owner.sendMessage(ChatColor.GREEN + "Setting username to " + event.getMessage());
                    break;

                case STRING2:
                    ZetaCore.INSTANCE.localSettings.databaseName = event.getMessage();
                ...
                case STRING3:
                    if (event.getMessage().equals("NO")) {
                        ...
                        ZetaCore.INSTANCE.localSettings.password = "";
                    } else {
                        ZetaCore.INSTANCE.localSettings.password = event.getMessage();
    ...
    public static class PluginEditMenu extends AbstractGUIMenu {
        ...
        @Override
        public void setItems() {
            if (!ZetaCore.hasSuperConfig(this.plugin)) {
                this.inv.setItem(11, makeItem(Material.REDSTONE_BLOCK, "&cDebug Logging", "", "&aDetermine whether this plugin should add in debug logs to console.", "&aThis can help when trying to find problems or reporting bugs", "&6" + ZetaCore.getBooleanColor(this.plugin.getSettings().isDebugLogging())));
                this.inv.setItem(13, makeItem(Material.BARRIER, "&6Disable", "", "&aDisables or enables this plugin.", "&aAfterwards you must restart the server to make the changes take effect.", "&6" + ZetaCore.getBooleanColor(this.plugin.getSettings().isDisabled())));
                this.inv.setItem(15, makeItem(Material.ENDER_CHEST, "&6Sync with database", "", "&aDetermines whether this plugin should sync with the database.", "&aThis overrides all over forced syncs including one-time syncs", "&6" + ZetaCore.getBooleanColor(this.plugin.getSettings().isSyncDatabase())));
            }
            else {
                this.inv.setItem(10, makeItem(Material.REDSTONE_BLOCK, "&cDebug Logging", "", "&aDetermine whether this plugin should add in debug logs to console.", "&aThis can help when trying to find problems or reporting bugs", "&6" + ZetaCore.getBooleanColor(this.plugin.getSettings().isDebugLogging())));
                this.inv.setItem(12, makeItem(Material.BARRIER, "&6Disable", "", "&aDisables or enables this plugin.", "&aAfterwards you must restart the server to make the changes take effect.", "&6" + ZetaCore.getBooleanColor(this.plugin.getSettings().isDisabled())));
                this.inv.setItem(14, makeItem(Material.ENDER_CHEST, "&6Sync with database", "", "&aDetermines whether this plugin should sync with the database.", "&aThis overrides all over forced syncs including one-time syncs", "&6" + ZetaCore.getBooleanColor(this.plugin.getSettings().isSyncDatabase())));
        ...
        public void handleClick(InventoryClickEvent e) {
            switch (e.getSlot()) {
                ...
                case 16:
                    Optional<AbstractGUIMenu> menuOptional = ZetaCore.getSuperConfig(this.plugin, this.menuUtil, this);
